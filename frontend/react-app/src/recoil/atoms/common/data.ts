...
const expectedScore = (ratingA: number, ratingB = 1000): number =>
...
export const createDummyUsers = (num: number): UserType[] => {
  return Array.from({ length: num }, (_, index): UserType => {
    const id = `42-10000${index + 1}`;
    const nickname = `user${Math.floor(Math.random() * 9) + 2}`;
    ...
    const rating = calculateRating(1000, ladder_win, ladder_lose);
